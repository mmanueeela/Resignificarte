...
========================================================= */
const LIMITE_PALABRAS = 24;

function prepararComentario(comentarioItem) {
    // Evitar inicializar el mismo comentario varias veces
    if (comentarioItem.dataset.verMasInicializado === "1") {
        return;
    }

    const textoElemento = comentarioItem.querySelector(".texto-comentario");
    if (!textoElemento) {
        return;
    }

    const textoCompleto = textoElemento.textContent.trim();
    const palabras = textoCompleto.split(/\s+/);

    // Si el comentario es corto, no hacemos nada
    if (palabras.length <= LIMITE_PALABRAS) {
        comentarioItem.dataset.verMasInicializado = "1";
        return;
    }

    const textoCorto = palabras.slice(0, LIMITE_PALABRAS).join(" ") + "…";

    textoElemento.dataset.textoCompleto = textoCompleto;
    textoElemento.dataset.textoCorto = textoCorto;
    textoElemento.textContent = textoCorto;

    // Crear botón "Ver más"
    const boton = document.createElement("button");
    boton.type = "button";
    boton.className = "btn-ver-mas-comentario";
    boton.textContent = "Ver más";

    comentarioItem.appendChild(boton);
    comentarioItem.dataset.verMasInicializado = "1";
}

function cerrarComentario(comentarioItem) {
    const textoElemento = comentarioItem.querySelector(".texto-comentario");
    const boton = comentarioItem.querySelector(".btn-ver-mas-comentario");

    if (!textoElemento || !boton) {
        return;
    }

    textoElemento.textContent = textoElemento.dataset.textoCorto;
    boton.textContent = "Ver más";
    comentarioItem.classList.remove("comentario-expandido");
}

function abrirComentario(comentarioItem) {
    const textoElemento = comentarioItem.querySelector(".texto-comentario");
    const boton = comentarioItem.querySelector(".btn-ver-mas-comentario");
    ...
    /*
     * Buscar únicamente dentro de SU columna.
     *
     * De esta manera puede haber:
     * 1 comentario Web abierto
     * +
     * 1 comentario RV abierto
     */
    const columna = comentarioItem.closest(".columna-comentarios");

    if (columna) {
        const comentarioAbierto = columna.querySelector(".comentario-item.comentario-expandido");

        if (comentarioAbierto && comentarioAbierto !== comentarioItem) {
            cerrarComentario(comentarioAbierto);
        }
    }

    textoElemento.textContent = textoElemento.dataset.textoCompleto;
    boton.textContent = "Ver menos";
    comentarioItem.classList.add("comentario-expandido");
}

function prepararTodosLosComentarios() {
    document.querySelectorAll(".comentario-item").forEach(prepararComentario);
}

// Inicializar comentarios existentes
prepararTodosLosComentarios();

// Click en Ver más / Ver menos
document.addEventListener("click", function (event) {
    const boton = event.target.closest(".btn-ver-mas-comentario");
    if (!boton) {
        return;
    }

    const comentarioItem = boton.closest(".comentario-item");
    if (!comentarioItem) {
        return;
    }

    if (comentarioItem.classList.contains("comentario-expandido")) {
        cerrarComentario(comentarioItem);
    } else {
        abrirComentario(comentarioItem);
    }
});

/*
 * Si después de publicar un comentario
 * tu web lo añade mediante JavaScript,
 * también se preparará automáticamente.
 */
const contenedorPrincipal = document.querySelector(".main-obras-artista");
...
if (contenedorPrincipal) {
    ...
    observer.observe(contenedorPrincipal, {
        childList: true,
        subtree: true
    });
}