...
function registrarUsuario(
    ...
    $consulta_insert_user = "
        INSERT INTO usuarios
        (nombre, apellidos, pais, fecha_nacimiento)
        VALUES (?, ?, ?, ?)
    ";
    ...
    if (!$stmt_user->execute()) {
        ...
        return [
            "exito" => false,
            "mensaje" => "Error al guardar el usuario: " . $error
        ];
    }
    ...
    // 4. Insertar datos en `usuarios_credenciales`
    //    Ahora también guardamos el teléfono
    $consulta_insert_cred = "
        INSERT INTO usuarios_credenciales
        (usuario_id, email, telefono, password)
        VALUES (?, ?, ?, ?)
    ";

    $stmt_cred = $conexion->prepare($consulta_insert_cred);

    $stmt_cred->bind_param(
        "isss",
        $nuevo_id,
        $email,
        $telefono,
    ...
    if ($stmt_cred->execute()) {
        ...
        return [
            "exito" => false,
            "mensaje" => "Error al guardar credenciales: " . $error
        ];
    }
}
